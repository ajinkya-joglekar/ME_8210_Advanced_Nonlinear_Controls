...
% system dynamics
lambda1=-2; lambda2=-2; %Linearization eigen values
f = @(t, x) [lambda1*(-x(1)+x(2,:).^2); lambda2*(-3*x(2,:)+3*x(1,:).^2)];
...
u = alpha.*(lambda1.*(-X+Y.^2));
v = alpha.*(lambda2.*(-3*Y+3*X.^2));

%% Path integral setup
% determine the eigenvalues and left-eigenvectors
x = sym('x',[2;1], 'real');
A = double(subs(jacobian(f(0,x),x),x,[0;0])); %Point corresponding to the mentioned eigenvalues
fn = @(x) f(0,x)-A*x;
[~,D,W] = eig(A); 
l1 = D(1,1); l2 = D(2,2);
w1 = W(:,1); w2 = W(:,2);
